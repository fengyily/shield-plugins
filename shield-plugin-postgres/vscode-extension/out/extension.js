...
class ConnectionStorage {
    context;
    key = 'shield-postgresql.connections';
    constructor(context) {
...
function activate(context) {
    ...
    const connectDisposable = vscode.commands.registerCommand('shield-postgresql.connect', async () => {
        ...
        const user = config.get('user', 'postgres');
        const password = config.get('password', '');
        const database = config.get('database', 'postgres');
    ...
    const addConnectionDisposable = vscode.commands.registerCommand('shield-postgresql.addConnection', async () => {
        ...
        const panel = vscode.window.createWebviewPanel('shield-postgresql.addConnection', 'Add PostgreSQL Connection', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    const removeConnectionDisposable = vscode.commands.registerCommand('shield-postgresql.removeConnection', async (item) => {
        if (item) {
    ...
    const editConnectionDisposable = vscode.commands.registerCommand('shield-postgresql.editConnection', async (item) => {
        if (item) {
            const connection = item.connection;
            // Create webview panel
            const panel = vscode.window.createWebviewPanel('shield-postgresql.editConnection', 'Edit PostgreSQL Connection', vscode.ViewColumn.One, {
                enableScripts: true,
                localResourceRoots: [vscode.Uri.joinPath(context.extensionUri, 'src')]
            });
            // Webview HTML content
    ...
    const connectFromTreeDisposable = vscode.commands.registerCommand('shield-postgresql.connectFromTree', async (item) => {
        if (item) {
            const connection = item.connection;
            ...
            if (connectionStatus[connection.id] === 'connected' && connectionInfo[connection.id]) {
                ...
                const panel = vscode.window.createWebviewPanel('shield-postgresql.connection', `PostgreSQL - ${connection.name}`, vscode.ViewColumn.One, {
                    enableScripts: true,
    ...
    const disconnectDisposable = vscode.commands.registerCommand('shield-postgresql.disconnect', async (item) => {
        if (item) {
            const connection = item.connection;
    ...
    const openInBrowserDisposable = vscode.commands.registerCommand('shield-postgresql.openInBrowser', async (item) => {
        if (item) {
            const connection = item.connection;
            // Check if connection is established
            if (connectionStatus[connection.id] === 'connected' && connectionInfo[connection.id]) {
    ...
    const testConnectionDisposable = vscode.commands.registerCommand('shield-postgresql.testConnection', async (item) => {
        if (item) {
            const connection = item.connection;
            // Get platform-specific executable name
            const platform = process.platform;
    ...
    // Add subscriptions
    context.subscriptions.push(connectDisposable, addConnectionDisposable, removeConnectionDisposable, editConnectionDisposable, connectFromTreeDisposable, testConnectionDisposable);
}
// Open PostgreSQL Web Client in browser
function openPostgreSQLWebView(context, port) {
    // Construct the URL
    const url = `http://localhost:${port}`;
